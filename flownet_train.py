...
#########################################################
#### As mentioned in paper, 1024 filters were considered. In the other code, as I told, I modified the code 
#### while experimenting to see the impact on output.
##########################################################       
...
class Net(object):
    def __init__(self, use_gpu_1=True):
        ...
        with tf.variable_scope('conv'):
            ...
            # predict# variable stores flow estimate at each level
            # to represent/indicate uv(flow in x and y dirn.) fields, 2 filters were used.
            predict6 = slim.conv2d(conv6_1, 2, [3, 3], 1, activation_fn=None, scope='pred6')

        with tf.variable_scope('deconv'):
            # Here, the output from 'conv' part is upsampled at each level till we get required resolution.
            # Instead of directly upsampling the 'conv' layer, a concatination step is performed which
            # considers the conv layer + upsampled previous conv layer + upsampled previous predicted flow field
            # You can see this in 'concat5' , 'concat4'... below
            
            # This concatination step helps in getting faster and better results than simply upsampling conv outputs
    ...
    with tf.Session(config=model.tf_config) as sess:
        ...
        for epoch in range(epoch_max):
            ...
            for iteration in range(iter_per_epoch):
                ...
                ## uncomment below 2 'plt' plotting lines and modify line 282 as below to see flow fields. You can also see flow fields at different levels
                
                ## modify line 282 to 
                # _, merged_out_t, loss_out_t, predict_out = sess.run([model.train_op, model.merged, model.loss, model.predict1], feed_dict)
